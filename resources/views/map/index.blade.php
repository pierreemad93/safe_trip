    ...
    @section('bottom_script')
        <script>
            $(function() {
                ...
                function initialize() {
                    var myLatlng = new google.maps.LatLng(29.95375640, 31.53700030);
                    var myOptions = {
                        zoom: 1.5,
                        center: myLatlng,
                    ...
                    if (locations.length > 0) {
                        for (i = 0; i < locations.length; i++) {
                            ...
                            if (markers[locations[i].id]) {
                                markers[locations[i].id].setMap(
                                    null); // set markers setMap to null to remove it from map
                            ...
                            marker = new google.maps.Marker({
                                position: new google.maps.LatLng(parseFloat(locations[i].latitude) + (Math
                                    .random() - .5) / 1500, parseFloat(locations[i].longitude) + (Math
                                    .random() - .5) / 1500),
                                map: map,
                                icon: taxicon,
                            ...
                            google.maps.event.addListener(marker, 'click', (function(marker, i) {
                                return function() {
                                    ...
                                    service_name = driver.driver_service != null ? driver.driver_service
                                        .name : '-';
                                    last_location_update_at = driver.last_location_update_at != null ?
                                        driver.last_location_update_at : '-';
                                    driver_view = "{{ route('driver.show', '') }}/" + marker
                                        .driver_id;
                                    contentString =
                                        '<div class="map_driver_detail"><ul class="list-unstyled mb-0">' +
                                        '<li><i class="fa fa-address-card" aria-hidden="true"></i>: ' +
                                        driver.display_name + '</li>' +
                                        '<li><i class="fa fa-phone" aria-hidden="true"></i>: ' + driver
                                        .contact_number + '</li>' +
                                        '<li><i class="fa fa-taxi" aria-hidden="true"></i>: ' +
                                        service_name + '</li>' +
                                        '<li><i class="fa fa-clock" aria-hidden="true"></i>: ' +
                                        last_location_update_at + '</li>' +
                                        '<li><a href="' + driver_view +
                                        '"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('message.view_form_title', ['form' => __('message.driver')]) }}</a></li>' +
                                        '</ul></div>';
